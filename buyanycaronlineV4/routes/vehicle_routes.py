...
async def create_images(_id, request):
    db_obj = AdvertisementDB.get_advertisement_by_id(_id)

    body = request.json
    images = body['vehicle'].get('images', [])

    index_of_images = 1
    advertisement_images = []

    for _image in images:
        file_name = '%s-%d' % (_id, index_of_images)
        file = File(name=file_name, type=_image['mime'], body=base64.decodebytes(str.encode(_image['data'])))
        img_id = ImageDB.create_image(file_name, file, _id)
        advertisement_images.append({'_id': str(img_id)})
        index_of_images += 1

    db_obj.set_images(advertisement_images)
    AdvertisementDB.update_advertisement(db_obj, _id)
...
async def create_car(request):
    ...
    result = AdvertisementDB.create_advertisement(car)

    if result[1]:
        await create_images(_id=result[1], request=request)
...
async def create_heavy(request):
    ...
    result = AdvertisementDB.create_advertisement(heavy)
...
async def create_boat(request):
    ...
    result = AdvertisementDB.create_advertisement(boat)
...
async def create_motorcycle(request):
    ...
    result = AdvertisementDB.create_advertisement(motorcycle)
